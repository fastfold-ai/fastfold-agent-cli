...
4. **Wait:** Do not run create job, wait, fetch, or download until the user confirms they have pasted and saved the key. After they confirm, run the scripts from the workspace root using the path `.claude/skills/fold/scripts/` (e.g. `python .claude/skills/fold/scripts/create_job.py ...`).
...
**Scripts:** Prefer the bundled scripts so behavior matches the SDK. From the **workspace root**, run them using the path `.claude/skills/fold/scripts/`. The agent should run these scripts for the user, not hand them a list of commands.

- **Create job (two modes):**
  - **Simple (single protein):** `python .claude/skills/fold/scripts/create_job.py --name "My Job" --sequence MALW... [--model boltz-2] [--public]`
  - **Full payload (same as [FastFold Python SDK](https://github.com/fastfold-ai/fastfold-python) / JobInput):** `python .claude/skills/fold/scripts/create_job.py --payload job.json` or `python .claude/skills/fold/scripts/create_job.py --payload -` (stdin). Payload must be JSON with `name`, `sequences`, `params`; optional `constraints` (pocket, bond), `isPublic`, and sequence types: `proteinChain`, `rnaSequence`, `dnaSequence`, `ligandSequence`. Use this for multi-chain, ligands, constraints, or custom params (e.g. `recyclingSteps`, `relaxPrediction`) so the agent does not need to write one-off scripts. Examples in [references/jobs.yaml](references/jobs.yaml).
- **Wait for completion:** `python .claude/skills/fold/scripts/wait_for_completion.py <job_id> [--poll-interval 5] [--timeout 900]`
- **Fetch results (JSON):** `python .claude/skills/fold/scripts/fetch_results.py <job_id>`
- **Download CIF:** `python .claude/skills/fold/scripts/download_cif.py <job_id> [--out output.cif]`
- **Viewer link:** `python .claude/skills/fold/scripts/get_viewer_link.py <job_id>`
