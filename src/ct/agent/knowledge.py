...
- **fold skill** (FastFold Cloud API): USE for running new protein structure predictions with FastFold models. Supported `modelName` values (exact strings): `boltz-2`, `openfold3`, `chai1`, `intellifold`, `monomer` (AlphaFold2 single-chain), `multimer` (AlphaFold2 complexes), `esm1b` (ESMFold — use this when the user says "ESM", "ESMFold", or "ESM-1b"), `boltz`, `simplefold_100M`, `simplefold_360M`, `simplefold_700M`, `simplefold_1.1B`, `simplefold_1.6B`, `simplefold_3B`. Invoke via the PATH console commands `fastfold-fold-create-job`, `fastfold-fold-wait-for-completion`, `fastfold-fold-fetch-results`, `fastfold-fold-download-cif`, `fastfold-fold-get-viewer-link`, `fastfold-fold-wait-for-evolla-linked`. Do NOT `python -m …`, `find`, `ls` the package directory, or probe `uv tool dir` — the console commands are always on PATH after `uv tool install fastfold-agent-cli`. `FASTFOLD_API_KEY` is already set in the environment.
