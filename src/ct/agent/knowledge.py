...
- **fold skill** (FastFold Cloud API): USE for running new protein structure predictions with FastFold models (boltz-2, openfold3, chai1, intellifold, monomer, multimer, simplefold_*). This is the preferred route for running fresh folds — it calls the FastFold Jobs API via the bundled Python modules `python -m ct.skills.fold.scripts.<script>` (create_job, wait_for_completion, fetch_results, download_cif, get_viewer_link, wait_for_evolla_linked). Do NOT search for them in `.claude/skills/` — they are shipped inside this CLI's Python package. `FASTFOLD_API_KEY` is already set in the environment.
- **md-simulation skill** (FastFold Workflows API): USE for running CALVADOS + OpenMM molecular dynamics simulations (workflow type `calvados_openmm_v1`). Bundled modules are `python -m ct.skills.md_simulation.scripts.<script>` (submit_from_fold_job, submit_manual_af_pae, fetch_uniprot, wait_for_workflow, fetch_results, toggle_public). When the user supplies a UniProt accession (e.g. `P00698`), first run `fetch_uniprot <ID> --out-dir /tmp/uniprot` to pull PDB + PAE from AlphaFold DB, then pipe them into `submit_manual_af_pae`. After completion these scripts print the Fastfold Cloud dashboard URL and the Py2DMol trajectory viewer link — forward them to the user. Do NOT search for them in `.claude/skills/`.
