...
- **fold skill** (FastFold Cloud API): USE for running new protein structure predictions with FastFold models. Supported `modelName` values (exact strings): `boltz-2`, `openfold3`, `chai1`, `intellifold`, `monomer` (AlphaFold2 single-chain), `multimer` (AlphaFold2 complexes), `esm1b` (ESMFold — use this when the user says "ESM", "ESMFold", or "ESM-1b"), `boltz`, `simplefold_100M`, `simplefold_360M`, `simplefold_700M`, `simplefold_1.1B`, `simplefold_1.6B`, `simplefold_3B`. Invoke via the PATH console commands `fastfold-fold-create-job`, `fastfold-fold-wait-for-completion`, `fastfold-fold-fetch-results`, `fastfold-fold-download-cif`, `fastfold-fold-get-viewer-link`, `fastfold-fold-wait-for-evolla-linked`, `fastfold-fold-wait-for-openmm-linked`. Do NOT `python -m …`, `find`, `ls` the package directory, or probe `uv tool dir` — the console commands are always on PATH after `uv tool install fastfold-agent-cli`. `FASTFOLD_API_KEY` is already set in the environment.
- Fold background protocol (required): never run `fastfold-fold-create-job` in background. Run submission in foreground, parse and return `job_id` immediately, and keep that exact `job_id` in follow-up context. Only long waits (`fastfold-fold-wait-for-completion`, `fastfold-fold-wait-for-evolla-linked`, `fastfold-fold-wait-for-openmm-linked`) may run in background. If a background wait completes, fetch results using the preserved `job_id` (do not ask the user to recover it). If `job_id` is missing due tool failure, re-run create-job foreground and return the new ID; do not use filesystem hunting (`find`, `locate`, shell history scraping, `/tmp` scans).
- **protein_design_boltzgen skill** (FastFold Workflows API): USE for BoltzGen protein design workflows and examples. Invoke via PATH console commands `fastfold-boltzgen-workflow` and `fastfold-boltzgen-fetch-cif`. Do NOT search the filesystem (`find`, `locate`, `ls` package paths) and do NOT `cd` into skill directories — run the console commands directly. If command execution fails, stop and ask user to upgrade `fastfold-agent-cli`; do not run fallback path-discovery commands.
...
- **md-openmm-calvados skill** (FastFold Workflows API): USE for running CALVADOS + OpenMM molecular dynamics simulations (workflow type `calvados_openmm_v1`). Invoke via PATH console commands: `fastfold-openmm-calvados-submit-from-fold-job`, `fastfold-openmm-calvados-submit-manual-af-pae`, `fastfold-openmm-calvados-submit-from-workflow`, `fastfold-openmm-calvados-submit-from-yml-refs` (advanced, only when explicitly requested), `fastfold-openmm-calvados-fetch-uniprot`, `fastfold-openmm-calvados-wait-for-workflow`, `fastfold-openmm-calvados-fetch-results`, `fastfold-openmm-calvados-toggle-public`, `fastfold-openmm-calvados-extract-frame`. When the user supplies a UniProt accession (e.g. `P00698`), first run `fastfold-openmm-calvados-fetch-uniprot <ID> --out-dir /tmp/uniprot` to pull PDB + PAE from AlphaFold DB, then pipe them into `fastfold-openmm-calvados-submit-manual-af-pae`. **Default to private** — never pass `--public` unless the user explicitly asked for a public / shared / sharable link. Only surface the `?shared=true` URL when the workflow is actually public. After completion, forward the dashboard + Py2DMol URLs printed by the scripts verbatim.
- OpenMM-CALVADOS background protocol (required): never background submit/execute commands. Submit in foreground, return `workflow_id` immediately, then background only `fastfold-openmm-calvados-wait-for-workflow`. Always fetch results with the preserved `workflow_id`; never ask user to recover IDs for agent-initiated runs.
- **md-openmmdl skill** (FastFold Workflows API): USE for OpenMMDL molecular dynamics workflows (workflow type `openmmdl_v1`) when the user has topology + ligand inputs, wants to prepare draft scripts, execute DRAFT workflows, or fetch OpenMMDL analysis outputs. Invoke via PATH console commands: `fastfold-openmmdl-submit-manual-topology-ligands`, `fastfold-openmmdl-prepare-script`, `fastfold-openmmdl-submit-from-workflow`, `fastfold-openmmdl-execute-workflow`, `fastfold-openmmdl-wait-for-workflow`, `fastfold-openmmdl-fetch-results`, `fastfold-openmmdl-extract-frame`, `fastfold-openmmdl-toggle-public`. **Default to private** — only expose shared/public URLs when the workflow is explicitly made public.
- OpenMMDL background protocol (required): submit/execute in foreground and return `workflow_id` immediately. Background only the waiter (`fastfold-openmmdl-wait-for-workflow`). Keep `workflow_id` in context for every follow-up and results fetch; do not attempt ID recovery via filesystem or shell-history scanning.
