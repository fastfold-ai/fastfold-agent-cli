...
- **md-simulation skill** (FastFold Workflows API): USE for running CALVADOS + OpenMM molecular dynamics simulations (workflow type `calvados_openmm_v1`). Invoke via PATH console commands: `fastfold-md-submit-from-fold-job`, `fastfold-md-submit-manual-af-pae`, `fastfold-md-fetch-uniprot`, `fastfold-md-wait-for-workflow`, `fastfold-md-fetch-results`, `fastfold-md-toggle-public`. When the user supplies a UniProt accession (e.g. `P00698`), first run `fastfold-md-fetch-uniprot <ID> --out-dir /tmp/uniprot` to pull PDB + PAE from AlphaFold DB, then pipe them into `fastfold-md-submit-manual-af-pae`. **Default to private** — never pass `--public` unless the user explicitly asked for a public / shared / sharable link. Only surface the `?shared=true` URL when the workflow is actually public. After completion, forward the dashboard + Py2DMol URLs printed by the scripts verbatim.
