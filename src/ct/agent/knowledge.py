...
- **md-simulation skill** (FastFold Workflows API): USE for running CALVADOS + OpenMM molecular dynamics simulations (workflow type `calvados_openmm_v1`). Bundled modules are `python -m ct.skills.md_simulation.scripts.<script>` (submit_from_fold_job, submit_manual_af_pae, wait_for_workflow, fetch_results, toggle_public). After completion these scripts print the Fastfold Cloud dashboard URL and the Py2DMol trajectory viewer link — forward them to the user. Do NOT search for them in `.claude/skills/`.
...
- USE md-simulation skill WHEN: "run MD", "molecular dynamics", "simulate this protein", "CALVADOS", "OpenMM simulation", "dynamics from fold job"
