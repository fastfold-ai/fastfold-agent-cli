...
- **fold skill** (FastFold Cloud API): USE for running new protein structure predictions with FastFold models (boltz-2, openfold3, chai1, intellifold, monomer, multimer, simplefold_*). Invoke via the PATH console commands `fastfold-fold-create-job`, `fastfold-fold-wait-for-completion`, `fastfold-fold-fetch-results`, `fastfold-fold-download-cif`, `fastfold-fold-get-viewer-link`, `fastfold-fold-wait-for-evolla-linked`. Do NOT `python -m …`, `find`, `ls` the package directory, or probe `uv tool dir` — the console commands are always on PATH after `uv tool install fastfold-agent-cli`. `FASTFOLD_API_KEY` is already set in the environment.
