"""
Small shared HTTP + multipart helpers for the md-openmmdl skill.
...
from ct.skills.md_openmmdl.scripts.security_utils import validate_results_payload
...
def upload_library_file_and_get_ref(
    ...
    *,
    ...
    base_name = (item_name or f"openmmdl-input-{file_path.stem}-{uuid.uuid4().hex[:8]}").strip()
...
DASHBOARD_RESULTS_URL_TEMPLATE = "https://cloud.fastfold.ai/openmmdl/results/{workflow_id}"
PUBLIC_SHARE_URL_TEMPLATE = "https://cloud.fastfold.ai/openmmdl/results/{workflow_id}?shared=true"
DEEP_ANALYSIS_URL_TEMPLATE = "https://cloud.fastfold.ai/openmmdl/results/md-analysis/{workflow_id}"
...
def build_result_links(workflow_id: str, *, is_public: bool = False) -> dict:
    """Return canonical cloud URLs for a completed OpenMMDL workflow."""
    return {
        ...
        "deep_analysis_url": DEEP_ANALYSIS_URL_TEMPLATE.format(workflow_id=workflow_id),
