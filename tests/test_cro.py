...
def mock_cro_db():
    with patch("tools.cro._load_cro_db", return_value=MOCK_CRO_DB):
        # Reset cache
        import tools.cro as cro_mod
...
class TestCroMatchExperiment:
    def test_match_cell_based_assay(self):
        from tools.cro import match_experiment
...
class TestCroDraftInquiry:
    def test_draft_inquiry_basic(self):
        from tools.cro import draft_inquiry
...
class TestCroSendInquiry:
    def test_send_inquiry_dry_run(self):
        from tools.cro import send_inquiry
        result = send_inquiry(
            cro_id="reaction-biology",
